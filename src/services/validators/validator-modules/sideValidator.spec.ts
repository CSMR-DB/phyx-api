...
describe('SideValidator()', () => {
  ...
  test('sideValidator() with imported mock data [valid]', async () => {
    await expect(
      sideValidator(csgoStrategyValid, csgoStrategyDataTransposer(csgoStrategyValid), csgoDataManager).execute()
    ).resolves.toEqual({ errors: [], result: true })
  })

  test('sideValidator() with imported mock data [invalid]', async () => {
    await expect(
      sideValidator(csgoStrategyInvalidSide, csgoStrategyDataTransposer(csgoStrategyInvalidSide), csgoDataManager).execute()
    ).resolves.toEqual({
      errors: [ Error('P2000 is not equippable on ATK side'), Error('M4A4 is not equippable on ATK side') ],
      result: false
    })
  })
})
