...
export function siegeGadgetValidator(
  ...
  async function execute(): Promise<boolean> {
    ...
    operators().map((operator: ISiegeOperator) => {
      const operatorData: R6SIEGE.IOperator | undefined = gameDataManager.getOneById(operator.internal_id)
      const hasValidGadget: boolean = operator.gadget.deployed_at.length <= operatorData!.gadget.count
