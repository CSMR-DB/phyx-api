...
export function csgoStrategyDataTransposer(strategy: ICSGOStrategy): IStrategyDataTransposer {
  const {
    team: { players }
  ...
  function uniqueIDs(): ICSGOItem['internal_id'][] {
    ...
    const loadoutArray: ICSGOLoadout[] = playersArray.map((player: ICSGOPlayer) => player['loadout'])
    ...
    const ids: ICSGOItem['internal_id'][] = items.map(({ internal_id }: { internal_id: string }) => internal_id)
