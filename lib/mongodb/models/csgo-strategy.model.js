...
const GearDetails = new mongoose_1.Schema({
    cost: Number,
    equipped: Boolean
});
const Utility = (price, max) => new mongoose_1.Schema({
    cost: Number,
    price: {
        type: Number,
        default: price
...
const Utilities = new mongoose_1.Schema({
    cost: Number,
    smoke: Utility(300, 1),
    flash: Utility(200, 2),
    frag: Utility(300, 1),
...
const Primary = new mongoose_1.Schema({
    cost: Number,
    name: String,
    category: {
        type: String,
...
const Gear = new mongoose_1.Schema({
    cost: Number,
    kevlar: GearDetails,
    helmet: GearDetails,
    zeus: GearDetails
});
const Secondary = new mongoose_1.Schema({
    cost: Number,
    name: String
});
...
const Player = (color) => new mongoose_1.Schema({
    color: {
        type: String,
        default: color
    },
...
const Players = new mongoose_1.Schema({
...
const MongooseModelCSGOStrategy = mongoose_1.default.model('csgo', schema, 'csgo');
exports.default = MongooseModelCSGOStrategy;
//# sourceMappingURL=csgo-strategy.model.js.map